...
class Pcindex
{
    ...
    {
        $list = Db::name('pcbanner')->order('id', 'desc')->select();
        if($list == null){
            return errMsg('数据为空');
        }else{
    ...
    public function bannerdel(Request $request)
    {
        ...
        $del = Db::name('pcbanner')->where('id', $request['id'])->delete();
        if($del == null){
    ...
    public function banneradd(Request $request)
    {
        ...
        $add = Db::name('pcbanner')->save($data);
        if($add == null){
    ...
    public function bannerupd(Request $request)
    {
        ...
        $add = Db::name('pcbanner')->where('id', $request['data']['id'])->save($data);
        if($add == null){
    ...
    public function bannerid(Request $request)
    {
        ...
        $bannerinfo = Db::name('pcbanner')->where('id', $request['id'])->find();
        if($bannerinfo == null){
            return errMsg('未查到相关信息');
        }else{
    ...
    public function bannertype(Request $request)
    {
        ...
        $bannerinfo = Db::name('pcbanner')->where('id', $request['id'])->save();
        if($bannerinfo == null){
            return errMsg('未查到相关信息');
        }else{
            return sucessMsg('成功',$bannerinfo);
