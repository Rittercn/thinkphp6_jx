...
class Cbanner
{
    ...
    {
        $list = Db::name('cbanner')->order('id', 'desc')->select();
        if($list == null){
            return errMsg('数据为空');
        }else{
    ...
    public function bannerdel(Request $request)
    {
        ...
        $del = Db::name('cbanner')->where('id', $request['id'])->delete();
        if($del == null){
    ...
    public function bannerupd(Request $request)
    {
        ...
        $add = Db::name('cbanner')->where('id', $request['data']['id'])->save($data);
        if($add == null){
    ...
    public function bannerid(Request $request)
    {
        ...
        $bannerinfo = Db::name('cbanner')->where('id', $request['id'])->find();
        if($bannerinfo == null){
            return errMsg('未查到相关信息');
        }else{
    ...
    public function bannertype(Request $request)
    {
        ...
        $bannerinfo = Db::name('cbanner')->where('id', $request['id'])->save();
        if($bannerinfo == null){
            return errMsg('未查到相关信息');
        }else{
            return sucessMsg('成功',$bannerinfo);
